...
valores = crit([float(d['value']) for d in data])

# Calcular el promedio, la varianza y la desviación típica
mean = sum(valores) / len(valores)
varianza = sum((x - mean) ** 2 for x in valores) / len(valores)
desv_tip = varianza ** 0.5
...
print("El promedio de las mediciones es:", mean, "±", round(error_abs, 2),"cm.")

# Calcular el tiempo promedio, pasando los cm del resultado a metros:
tiempo_prom = ((2 * (mean * 0.01) / 9.8) ** 0.5)

# Calcular el error asociado al tiempo, pasando los cm del resultado a metros:
tiempo_error = ((2 / 9.8) ** 0.5) * ((error_abs * 0.01) / (2 * ((mean * 0.01) ** 0.5)))
